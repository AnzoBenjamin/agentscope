...
export async function investigateSessionWithSplunk(input: {
  ...
  //
  // SECURITY: `input.sessionId` is generated by `randomUUID()` upstream
  // (see `agent.ts:execute()` and the run-queue claim path), so the only
  // characters that can ever appear in it are `[0-9a-f-]`. The escape
  // below is therefore cosmetic; it is NOT general-purpose SPL
  // injection protection. If this signature ever changes to accept a
  // caller-supplied sessionId, replace this with a strict allowlist
  // check (`/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i`)
  // and reject otherwise, instead of trying to escape.
  ...
  // Hoist `extractEventTypes` so `buildFindings` and `inferRiskLevel`
  // don't each re-derive the same Set from the same `rawResults`. Both
  // helpers run back-to-back here, so we share the work.
  const eventTypes = extractEventTypes(rawResults);
...
function buildFindings(
  ...
  // Pre-computed event type set; passed in by the caller to avoid
  // re-deriving the same Set from the same `rawResults` on every call.
  // Falls back to a fresh extraction if not provided so this helper is
  // still usable in isolation (e.g. from tests).
  eventTypes: Set<string> = extractEventTypes(rawResults),
): string[] {
  ...
  const hasFailure = eventTypes.has("SessionFailed");
  const hasCost = eventTypes.has("CostRecorded");
  findings.push(
    "AgentScope recorded replayable events for model, tool, cost, and completion signals.",
  );

  if (hasFailure) {
    findings.push("A `SessionFailed` event was present in the Splunk result.");
  } else {
    findings.push(
      "No `SessionFailed` event was present in the Splunk result.",
    );
  }
...
function inferRiskLevel(
  ...
  // Risk is computed from typed event types, not by substring-scanning
  // the serialized payload. A field like `errorCount: 0` or `errors: []`
  // in a successful event would otherwise trip a false-positive
  // `high` risk. The risk is `high` only if the session actually
  // emitted a `SessionFailed` event.
  if (eventTypes.has("SessionFailed")) {
    return "high";
  }
...
/**
 * Pull the set of event types present in a Splunk MCP result. The MCP
 * response may be the raw `{ eventCount, events: [...] }` object or
 * wrapped in an MCP `text` content envelope; both shapes are unwrapped
 * the same way `extractMcpResult` does it.
 *
 * Each event row is expected to have an `eventType` string field
 * (AgentScope's telemetry schema). Rows that don't are silently
 * ignored — we don't want a schema drift on a third-party field to
 * false-positive the risk level.
 */
function extractEventTypes(rawResults: unknown): Set<string> {
  const unwrapped = unwrapMcpResult(rawResults);
  if (unwrapped === null || typeof unwrapped !== "object") return new Set();
  const events = (unwrapped as { events?: unknown }).events;
  if (!Array.isArray(events)) return new Set();
  const types = new Set<string>();
  for (const row of events) {
    if (row && typeof row === "object") {
      const t = (row as { eventType?: unknown }).eventType;
